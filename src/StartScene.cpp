...
void StartScene::m_updateUI()
{
	...
	if (m_displayAbout)
	{
		ImGui::Begin("About Physics Assignment 2..", &m_displayAbout, ImGuiWindowFlags_AlwaysAutoResize);
		ImGui::Separator();
		ImGui::Text("Authors:");
		ImGui::Text("Amer Ali Mohammed");
		ImGui::Text("Talhah Ramadan Salick");
		ImGui::End();
	}
	...
	ImGui::PushItemWidth(80);
	if (ImGui::SliderFloat("Launching angle", &m_angle, 1.0f, 90.0f, "%.1f"))
	{

	}
	ImGui::PushItemWidth(80);
	if (ImGui::SliderFloat("Velocity", &m_velocity, 1.0f, 200.0f, "%.1f"))
	{
...
{
	m_velocity = m_velocity; // muzzle velocity
	// velocity components.
	...
	//Physics Equation
	m_finalPosition = m_pShip->getPosition() + (velocity_vector * m_time) + ((m_acceleration *0.5f) * (m_Atime * m_Atime));
	m_Atime += m_time;
